...
async def login_for_access_token(
    ...
    result=await db.execute(
        select(models.User)
        .where(
            func.lower(models.User.email)==form_data.username.lower() ##NOTE: here form_data takes input as username but we treat it as email
        ),
    )
...
##API to get current user data to frontend GET
...
async def forgot_password(
    ...
    return{
        "message":"check your registered email to for password reset link."
    }
...
##API change password for logged in user PATCH
...
##API upload profile picture PATCH
@router.patch("/{user_id}/picture",response_model=UserPrivate)
async def upload_profile_picture(
    user_id:int,
    file:UploadFile,##FastAPI object to handle file uploads
    current_user:CurrentUser, 
    db:Annotated[AsyncSession,Depends(get_db)],
):
